...
public sealed class HerdDataController(HerdDataImportService importer, ApplicationDbContext context, HerdDataAdminAccess admin) : ControllerBase
{
    ...
    public async Task<IActionResult> Analytics(CancellationToken ct)
    {
        ...
        var lifetimeProduction = await context.Set<LifetimeProductionSnapshot>().AsNoTracking().Include(snapshot => snapshot.Animal)
            .OrderByDescending(snapshot => snapshot.ReportDate).Select(snapshot => new { snapshot.AnimalId, AnimalName = snapshot.Animal.DisplayName, snapshot.Animal.SireName, snapshot.ReportDate, snapshot.LifetimeMilk, snapshot.LifetimeFat, snapshot.LifetimeProtein, snapshot.Lactations }).ToListAsync(ct);
        ...
        return Ok(new { latestMilkDate = milkDate, latestGenomicDate = genomicDate, milk, sireMilk, milkHistory, lifetimeProduction, genomic, bulls, genomicHistory, combined, attention = new { highDimOpen, longOpenHeifers, droppingMilk, dryOffWatch } });
    }
