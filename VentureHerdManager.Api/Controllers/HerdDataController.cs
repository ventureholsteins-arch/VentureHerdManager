...
public sealed class HerdDataController(HerdDataImportService importer, ApplicationDbContext context, HerdDataAdminAccess admin) : ControllerBase
{
    ...
    public async Task<IActionResult> Analytics(CancellationToken ct)
    {
        ...
        var lifetimeProduction = await context.Set<LifetimeProductionSnapshot>().AsNoTracking().Include(snapshot => snapshot.Animal)
            .OrderByDescending(snapshot => snapshot.ReportDate).Select(snapshot => new { snapshot.AnimalId, AnimalName = snapshot.Animal.DisplayName, snapshot.ReportDate, snapshot.LifetimeMilk, snapshot.LifetimeFat, snapshot.LifetimeProtein, snapshot.Lactations }).ToListAsync(ct);
