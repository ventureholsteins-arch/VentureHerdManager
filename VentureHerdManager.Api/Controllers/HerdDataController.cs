...
public sealed class HerdDataController(HerdDataImportService importer, ApplicationDbContext context, HerdDataAdminAccess admin) : ControllerBase
{
    ...
    public async Task<IActionResult> Analytics(CancellationToken ct)
    {
        ...
        var milk = records.Where(r => r.Source == HerdDataSource.Pcdart && r.ReportDate == milkDate).OrderByDescending(r => r.Milk).Select(r => new { r.AnimalId, AnimalName = r.Animal.DisplayName, r.Animal.SireName, r.Animal.CurrentLactation, r.ReportDate, r.DaysInMilk, r.Milk, r.FatPercent, r.ProteinPercent, FatPounds = r.Milk.HasValue && r.FatPercent.HasValue ? Math.Round(r.Milk.Value * r.FatPercent.Value / 100m, 2) : (decimal?)null, ProteinPounds = r.Milk.HasValue && r.ProteinPercent.HasValue ? Math.Round(r.Milk.Value * r.ProteinPercent.Value / 100m, 2) : (decimal?)null }).ToList();
        ...
        return Ok(new { latestMilkDate = milkDate, latestGenomicDate = genomicDate, milk, sireMilk, milkHistory, genomic, bulls, genomicHistory, combined, attention = new { highDimOpen, longOpenHeifers, droppingMilk, dryOffWatch } });
    }
