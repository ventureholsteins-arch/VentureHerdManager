...
import { applyHerdData, getHerdDataAnalytics, previewHerdData, setAdminKey, type HerdDataPreview, type HerdDataSource } from '../api/herdData'
...
async function unlock() {
  ...
  try {
    ...
    unlocked.value = true
    status.value = ''
  } catch (error) { status.value = error instanceof Error ? error.message : 'Admin access failed.' }
}
...
<template>
  <main class="data-page">
    ...
    <section v-if="!unlocked" class="card unlock"><h2>Admin access</h2><input v-model="adminKey" type="password" placeholder="Admin key"><button @click="unlock">Unlock private data</button><p v-if="status" class="error">{{ status }}</p></section>
    <template v-else>
