...
function genomicBoards(traits: Array<{ key: string; label: string; lower?: boolean }>) {
  return traits.map(trait => {
    const ranked = [...genomicRanked.value].filter((row: any) => row[trait.key] != null).sort((a: any, b: any) => trait.lower ? a[trait.key] - b[trait.key] : b[trait.key] - a[trait.key])
    const values = ranked.map((row: any) => Number(row[trait.key]))
    return { ...trait, average: values.length ? values.reduce((sum: number, value: number) => sum + value, 0) / values.length : null, best: ranked[0], watch: ranked.at(-1) }
  })
}
const fertilityHealthBoards = computed(() => genomicBoards([
  { key: 'heiferConceptionRate', label: 'Heifer Conception (HCR)' }, { key: 'cowConceptionRate', label: 'Cow Conception (CCR)' },
  { key: 'fertilityIndex', label: 'Fertility Index (FI)' }, { key: 'productiveLife', label: 'Productive Life (PL)' },
  { key: 'livability', label: 'Livability (LIV)' }, { key: 'healthCostIndex', label: 'Health Cost Index (HCC)' }
]))
const calvingBoards = computed(() => genomicBoards([
  { key: 'sireCalvingEase', label: 'Sire Calving Ease (SCE)', lower: true }, { key: 'daughterCalvingEase', label: 'Daughter Calving Ease (DCE)', lower: true },
  { key: 'sireStillbirth', label: 'Sire Stillbirth (SSB)', lower: true }, { key: 'daughterStillbirth', label: 'Daughter Stillbirth (DSB)', lower: true },
  { key: 'gestationLength', label: 'Gestation Length (GL)', lower: true }, { key: 'earlyFirstCalving', label: 'Early First Calving (EFC)' }
]))
  ...
  <main class="data-page analytics-stat-desk" :class="`${activeView}-desk`">
    ...
    <nav class="analytics-tabs" aria-label="Analytics sections">
      ...
      <section v-if="activeView === 'attention'" class="attention-grid">
        ...
        <article class="card attention-card"><h2>High-DIM Confirmed PG — Dry-Off Watch <b>{{ analytics?.attention?.dryOffWatch?.length ?? 0 }}</b></h2><p>Confirmed pregnant cows due to dry within 60 days. Shows DIM, current milk, and any recent milk drop.</p><button v-for="row in analytics?.attention?.dryOffWatch ?? []" :key="row.animalId" class="attention-row" @click="router.push(`/animals/${row.animalId}`)"><strong>{{ row.animalName }}</strong><span>{{ row.daysInMilk ?? '—' }} DIM · PG confirmed · {{ row.milk ?? '—' }} milk<span v-if="row.milkDropPercent"> · down {{ row.milkDropPercent }}%</span></span><small>{{ row.daysUntilDry < 0 ? `${Math.abs(row.daysUntilDry)} days overdue` : `${row.daysUntilDry} days` }} · dry {{ new Date(row.recommendedDryOffDate).toLocaleDateString() }}</small></button><small v-if="!(analytics?.attention?.dryOffWatch?.length)">No confirmed pregnant high-DIM cows are inside the 60-day window.</small></article>
      </section>
    <div v-show="activeView === 'imports'" class="import-choice">
      ...
      <section v-if="activeView === 'genomics'" class="genomic-dashboard">
        <details class="card"><summary>Fertility, longevity &amp; health comparisons</summary><p>Higher values are treated as better for these traits.</p><div class="trait-card-grid"><article v-for="trait in fertilityHealthBoards" :key="trait.key"><small>{{ trait.label }} herd average</small><strong>{{ trait.average?.toFixed(2) ?? '—' }}</strong><span class="strength">Leader: <button @click="trait.best && router.push(`/animals/${trait.best.animalId}`)">{{ trait.best?.animalName ?? '—' }} {{ trait.best?.[trait.key] ?? '' }}</button></span><span class="weakness">Watch: <button @click="trait.watch && router.push(`/animals/${trait.watch.animalId}`)">{{ trait.watch?.animalName ?? '—' }} {{ trait.watch?.[trait.key] ?? '' }}</button></span></article></div></details>
        <details class="card"><summary>Calving &amp; stillbirth comparisons</summary><p>Lower is generally preferred for SCE, DCE, SSB, DSB, and gestation length. EFC is ranked higher-first.</p><div class="trait-card-grid"><article v-for="trait in calvingBoards" :key="trait.key"><small>{{ trait.label }} herd average</small><strong>{{ trait.average?.toFixed(2) ?? '—' }}</strong><span class="strength">Best: <button @click="trait.best && router.push(`/animals/${trait.best.animalId}`)">{{ trait.best?.animalName ?? '—' }} {{ trait.best?.[trait.key] ?? '' }}</button></span><span class="weakness">Watch: <button @click="trait.watch && router.push(`/animals/${trait.watch.animalId}`)">{{ trait.watch?.animalName ?? '—' }} {{ trait.watch?.[trait.key] ?? '' }}</button></span></article></div></details>
      </section>
